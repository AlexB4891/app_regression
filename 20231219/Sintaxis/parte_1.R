  ...
  c(
    ...
    map(~{
      ...
      reg_tables <- list(
        ...
        cit_more_stat = reg_table_0 %>% 
          mutate(statutory = if_else(anio_fiscal == 2012, 0.23, 0.22)) %>% 
          filter(tasa_ir >= statutory),
        cit_less_stat = reg_table_0 %>% 
          mutate(statutory = if_else(anio_fiscal == 2012, 0.23, 0.22)) %>% 
          filter(tasa_ir < statutory & tasa_ir > 0 )
      )
      ...
      archivos <- names(results) %>% 
        str_c("03_RESULTADOS/WB_PB_INVERSIONES/20231219/Tablas/",.,"/dep_var_",.x,grupo,".txt")
