...
coef_plot_plus_se <- function(tidy_tems,
  ...
  tidy_tems %>% 
    mutate(term = str_remove_all(term,"anio_fiscal::"),
...
reg_table <- reg_table %>% 
  mutate(
    log_assets_attr_pff = log(total_assets*pff_p+ 1),
    log_assets_attr_ext = log(total_assets*ext_p+ 1),
    log_utility = log(net_profits+ 1),
    log_cit_liability = log(tax_liability+ 1),
    log_taxable_profits = log(taxable_profits+ 1),
    log_assets_prominent = log(prominent*total_assets+ 1)
  )
...
# 
# modelo <- diff_diff_model(rtable = reg_table %>% mutate(treatment = if_else(treatment == "Treatment",1,0))  ,dependent = "pff_p")
# 
# vari <- "pff_p"
# 
# tests <- modelo %>% 
#   imap(~perform_model_temp(.x,rtable = reg_table,label = .y,dependent = vari)) 
# 
# 
# tests$`Event study`$params %>% 
#   coef_plot_plus_se(title_plot = "Participacion en paraisos fiscales",
#                  subtitle_plot = "Event study design with fixed effects (assets weighted)")
...
execute_model <- function(grupo){
  ...
  c("any_change",
    ...
    map(~{
      ...
      results <- diff_diff_model(rtable = reg_table_0,
                                 dependent = vari) %>% 
        imap(~perform_model_temp(.x,rtable = reg_table_0,
                            label = .y,dependent = vari)) %>% 
        transpose()
      ...
      archivos <- names(results) %>% 
        str_c("03_RESULTADOS/WB_PB_INVERSIONES/20231115/Tablas/",.,"/dep_var_",.x,grupo,".txt")
...
# execute_model("_joint")

execute_model("_majors")

execute_model("_minors")
